...
const properties: Property[] = [
  ...
  },
  {
    ...
    title: "New York, USA",
    price: "$5,200,00",
    image: "/images/Assured-2.png",
  },
  {
    id: 3,
    title: "New York, USA",
    price: "$5,200,00",
    image: "/images/Assured-3.png",
  },
  {
    id: 4,
    title: "New York, USA",
    price: "$5,200,00",
    image: "/images/Assured-4.png",
  },
];
...
export default function HomePage() {
  ...
  useEffect(() => {
    // ✅ IntersectionObserver → reveal on mobile scroll
  ...
  return (
    <main className="w-full max-w-[1520px] lg:mb-[50px] mx-auto px-4 sm:px-6 lg:px-0">
      ...
      <section className="py-10 sm:py-14 lg:py-0">
        ...
        {/* ✅ Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-6 justify-center">
          {properties.map((p) => (
            <div
              key={p.id}
              data-id={p.id}
            ...
            >
              {/* Property Image */}
              <div
                className="relative w-full h-[220px] sm:h-[250px] lg:h-[280px] cursor-pointer"
              ...
              {/* ✅ Buttons → Mobile (click/scroll) + Tablet/Desktop (hover with animation) */}
              <div
                className={`
                  absolute bottom-0 left-0 w-full px-4 pb-3 flex gap-3 sm:gap-4 justify-center
                  transition-all duration-500
                  ${
                    // ✅ Mobile → based on scroll/click
                    visibleButtons.includes(p.id)
                      ? "translate-y-0 opacity-100 animate-slideUp"
                      : "translate-y-full opacity-0"
                  }
                  ${
                    // ✅ Tablet & Desktop → animate on hover
                    "sm:opacity-0 sm:translate-y-full sm:group-hover:opacity-100 sm:group-hover:translate-y-0 sm:group-hover:animate-slideUp"
                  }
                `}
              >
